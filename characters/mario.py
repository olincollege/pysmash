...
class Mario(Player):
    ...
    def __init__(self):
        ...
        smash_r = pygame.transform.scale(
            self.spritesheet.image_at((73, 303, 47, 43), (47, 54, 153)), (96, 86)
        )
    ...
    def set_boxes(self):
        ...
        if self.attack_cooldown > 0:
            ...
            if self.attack == 'smash':
                if self.direction == "left":
                    self.hurtbox = pygame.Rect(self.rect.x + 50, self.rect.y + 10, 46, 76)
                    self.hitbox = pygame.Rect(self.rect.x, self.rect.y-15, 59, 109)
                else:
                    self.hurtbox = pygame.Rect(self.rect.x, self.rect.y + 10, 46, 76)
                    self.hitbox = pygame.Rect(self.rect.x + 35, self.rect.y-15, 59, 109)
        else:
            self.hurtbox = pygame.Rect(self.rect.x, self.rect.y, 50, 76)
